...
class Client:
    def __init__(self, master):
        ...
        self.username = None
        self.socket = None
# tk .frame is used to create a child window from the main windwo
        self.message_frame = tk.Frame(master)
        self.message_frame.pack()
# k.Scrollbar(self.message_frame) is used to create a scrollbar widget that
#  can be used to scroll through the contents of another widge
        self.scrollbar = tk.Scrollbar(self.message_frame)
        self.scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        self.message_list = tk.Listbox(
            self.message_frame, yscrollcommand=self.scrollbar.set)
        ...
        self.connect_button = tk.Button(
            self.input_frame, text="Connect", command=self.connect)
        self.connect_button.pack(side=tk.LEFT)

        self.send_button = tk.Button(
            self.input_frame, text="Send", command=self.send_message, state=tk.DISABLED)
        self.send_button.pack(side=tk.RIGHT)

        self.input_entry = tk.Entry(
            self.input_frame, width=50, state=tk.DISABLED)
