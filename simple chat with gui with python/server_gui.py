...
# then the following is to allow the reuse of the same local address
...
def rec():
    ...
    while True:
        w = c.recv(2048)
        Button(wind, text=w.decode('utf-8'), bd=5, relief=FLAT, bg='#d12e44',
               padx=5, pady=5, wraplength=100).grid(column=2, row=r)
        r = r+1


# the embty tuble means it takes no arguments
...
def clicked():
    ...
    x = en.get()
    Button(wind, text=x, bd=5, relief=FLAT, bg='#d1c62e',
